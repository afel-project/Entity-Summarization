...
public final class ClusterStats {
     ...
     * @return the Bayes Information Criterion.
     */
    public static double computeBIC(final SparseTupleList tuples, final List<Cluster> clusters) {
     ...
     * @return the Bayes Information Criterion.
     */
    public static double computeBIC(final SparseTupleList tuples, final Cluster cluster) {
        return computeBIC(tuples, new Cluster[]{cluster});
    }
     ...
     * @return the Bayes Information Criterion.
     */
    public static double computeBIC(final SparseTupleList tuples, final Cluster[] clusters) {
        return computeBIC(tuples, Arrays.asList(clusters));
    }
    ...
    private static double computeDistortion(final SparseTupleList tuples, final Cluster cluster) {
        return cluster.getMemberCount() * TupleMath.norm1(computeVariance(tuples, cluster));
    }
    ...
    private static double[] computeVariance(final SparseTupleList tuples, final Cluster cluster) {
        int n = cluster.getMemberCount();
        double[] center = cluster.getCenter();
        ...
        if (n > 0) {
            ...
            for (int i = 0; i < n; i++) {
                TIntDoubleMap feature_vector = tuples.getTuple(i);
                for (int key : feature_vector.keys()) {
                    double value = feature_vector.get(key);
                    sum[key] += value;
                    variance[key] += value * value;
                    nonNaNCount[key]++;
                }
            }
