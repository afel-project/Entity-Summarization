 ...
 *
 *===================================================================*/
public class SparseKMeansSplittingParams {

    private int minClusters = 1;
    private int maxClusters = Integer.MAX_VALUE;
    private double minClusterToMeanThreshold = 0.05;
    private SparseDistanceMetric distanceMetric = new SparseEuclideanDistanceMetric();
    private SparseClusterSeeder clusterSeeder = new SparseKMeansPlusPlusSeeder(new SparseEuclideanDistanceMetric());
    private int workerThreadCount = Runtime.getRuntime().availableProcessors();

    public SparseKMeansSplittingParams() {
    }
     ...
     */
    public void setClusterSeeder(final SparseClusterSeeder seeder) {
        if (seeder == null) {
            throw new NullPointerException();
        }
