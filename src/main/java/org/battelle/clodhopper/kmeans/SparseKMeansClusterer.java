 ...
 *
 *===================================================================*/
public class SparseKMeansClusterer extends AbstractClusterer {
    ...
    public SparseKMeansClusterer(SparseTupleList tuples, SparseKMeansParams params) {
    ...
    public List<Cluster> doTask() throws Exception {
        ...
        try {
            ...
            if (actualClusterCount == 1) {
                ...
                final int workerCount = params.getWorkerThreadCount() > 0 ? params.getWorkerThreadCount() : Runtime.getRuntime().availableProcessors();
            ...
            if (emptyClustersDeleted > 0) {
                ph.postMessage(String.format("number of clusters was reduced to %d, because of %d clusters which became empty", clusters.size(), emptyClustersDeleted));
            }
    ...
    private void initializeCenters(ProgressHandler ph) {
        ...
        int uniqueTupleCount = tuples.getTupleCount();
        ...
        SparseTupleList seeds = seeder.generateSeeds(tuples, clusterCount);
        ...
        for (int i = 0; i < seedCount; i++) {
            ...
            TIntDoubleMap tmp_center = seeds.getTuple(i);
            for (int key : tmp_center.keys()) {
                center[key] = tmp_center.get(key);
            }
    ...
    private ProtoCluster[] split(ProtoCluster cluster, ProgressHandler ph) {
        ...
        SparseFilteredTupleList filteredTuples = new SparseFilteredTupleList(memberIndexes, tuples);

        SparseClusterSeeder seeder = params.getClusterSeeder();

        SparseKMeansParams splitterParams = new SparseKMeansParams.Builder().
                clusterCount(2).
        ...
        SparseKMeansClusterer splitter = new SparseKMeansClusterer(filteredTuples, splitterParams);
        splitter.run();
    ...
    private int nearestCluster(int tupleNdx, SparseDistanceMetric distMetric) {
        ...
        TIntDoubleMap buffer = tuples.getTuple(tupleNdx);
    ...
    private class SubtaskManager {
        ...
        private class AssignmentWorker implements Callable<Void> {
            ...
            private AssignmentWorker(int startTuple, int endTuple) {
                ...
                this.distanceMetric = params.getDistanceMetric().clone();
            }
            ...
            public Void call() throws Exception {
                try {
                    ...
                    for (int i = startTuple; i < endTuple; i++) {
                        int c = nearestCluster(i, distanceMetric);
                        if (c >= 0) {
    ...
    private static class ProtoCluster {
        ...
        private ProtoCluster(int[] members, double[] center) {
            this.currentMembers = members.clone();
            this.center = center;
        }
        ...
        private void updateCenter(SparseTupleList tuples) {
