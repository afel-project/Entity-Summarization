 ...
 *===================================================================*/
/**
 * An abstract base class that can be used for implementations of
 * <code>TupleList</code>
 *
 * @author R. Scarberry
 * @since 1.0
 *
 */
public abstract class SparseAbstractTupleList implements SparseTupleList{
     ...
     */
    protected SparseAbstractTupleList(final int tupleLength, final int tupleCount) {
        if (tupleLength < 0) {
            throw new IllegalArgumentException("tupleLength < 0: " + tupleLength);
        }
        if (tupleCount < 0) {
            throw new IllegalArgumentException("tupleCount < 0: " + tupleCount);
        }
     ...
     */
    protected void checkTupleIndex(final int n) {
        if (n < 0 || n >= tupleCount) {
    ...
    /**
     * Checks the column index, throwing an IndexOutOfBoundsException if it is
     * invalid.
     *
     * @param col the column index to check.
     */
    protected void checkColumnIndex(final int col) {
    ...
    /**
     * Checks to ensure the specified values array is long enough to hold tuples
     * from this object.
     *
     * @param values the array whose length to check.
     */
    protected void checkValuesLength(final double[] values) {
