...
public class PrecisionEvaluationSingle {
    private static Map<Pair<String, String> , pePredicate> _gt;

    public PrecisionEvaluationSingle(String path, String id, String type, double confidience) throws IOException {
        _gt = new HashMap<Pair<String, String> , pePredicate>();
        if(path.endsWith(".json")){
            readGTjson(path, id, type, confidience);
        }
        else if(path.endsWith(".tsv")){
            readGTtsv(path, id, type, confidience);
        }
    }
    private void readGTtsv(String path, String id, String type, double Conf) throws IOException {
        ...
        while( (line = br.readLine() ) != null){
            ...
            Pair<String,String> key = new Pair<>(segs[0], pred);
    ...
    private void readGTjson(String path, String id,String type, double Conf) throws IOException {
...
//        BufferedWriter output = new BufferedWriter(new FileWriter(new File("Movie.gt"), false));
        ...
        while ((line = br.readLine()) != null) {
          //  System.out.println(line);
            ...
            if (!query_id.equals(id)) continue;
