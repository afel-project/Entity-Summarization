	...
	public static Dataset loadSparse(Reader in, int classIndex, String attSep, String indexSep) {
	 ...
	 public static Dataset load(Reader in, int classIndex, String separator) {
	        ...
	        for (String line : it) {
	            ...
	            double[] values;
				String[] properties = new String[classIndex];
//				if(classIndex>0)
//					properties = new String[classIndex];
	            if (classIndex == -1)
	                values = new double[arr.length];
	            else
	                values = new double[arr.length - classIndex - 1];
...
//	                        values[i] = val;
	            ...
	            out.add(new FactDenseInstance(properties, values, classValue));

	        }
	        return out;
	    }

	public static Dataset load(Reader in, int classIndex, String separator, Integer startq, Integer endq) {
		...
		for (String line : it) {
			String[] arr = line.split(separator);
			if(arr.length <= classIndex) continue;
			if(Integer.valueOf(arr[0]) >= startq && Integer.valueOf(arr[0]) < endq) continue;
