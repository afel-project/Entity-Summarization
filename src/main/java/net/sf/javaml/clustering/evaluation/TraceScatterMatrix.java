...
public class TraceScatterMatrix implements ClusterEvaluation {
    ...
    public double score(Dataset[] clusters) {
        ...
        double sum = 0;
        for (int i = 0; i < clusters.length; i++) {
//            System.out.println("clusterCentroid[i]"+clusterCentroid[i].noAttributes());
//            System.out.println("overAllCentroid"+overAllCentroid.noAttributes());
            if(clusterCentroid[i].noAttributes()==0){
                clusterCentroid[i] = new DenseInstance(new double[overAllCentroid.noAttributes()]);
            }
